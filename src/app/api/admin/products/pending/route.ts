...
export async function GET() {
  ...
  // Include direct temporary link endpoint for admin preview (token-less logical URL; not the actual Dropbox link)
  const shaped = pending.map(p => ({
    id: p.id,
    title: p.title,
    ...
    // Admin can see the endpoint to generate final temporary link after purchase flow
    tempLinkEndpoint: `/api/download/[token]` // documentation hint; actual token is issued post-purchase
  }))
